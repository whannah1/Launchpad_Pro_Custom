...
# Embedded file name: /Users/versonator/Jenkins/live/output/mac_64_static/Release/python-bundle/MIDI Remote Scripts/_Framework/ArrangementComponent.py
...
class ArrangementComponent(CompoundComponent):
    u"""
    ...
    def __init__(self, num_tracks=0, num_scenes=0, auto_name=False, enable_skinning=False, *a, **k):
        super(ArrangementComponent, self).__init__(*a, **k)
        assert num_tracks >= 0
        assert num_scenes >= 0
        ...
        self._vertical_banking, self._vertical_paginator = self.register_components(ScrollComponent(), ScrollComponent())
    ...
    def set_track_bank_buttons(self, down_button, up_button):
        self.set_track_bank_up_button(up_button)
        self.set_track_bank_down_button(down_button)

    def set_track_bank_up_button(self, button):
        self._bank_up_button = button
        self._vertical_banking.set_scroll_up_button(button)

    def set_track_bank_down_button(self, button):
        self._bank_down_button = button
        self._vertical_banking.set_scroll_down_button(button)
    ...
    def set_offsets(self, track_offset, scene_offset):
        ...
        scene_increment = 0
        if self._is_linked():
            ArrangementComponent._perform_offset_change(track_offset - self._track_offset, scene_offset - self._scene_offset)
        elif len(self.tracks_to_use()) > track_offset:
    ...
    def update(self):
        super(ArrangementComponent, self).update()
    ...
    def _snap_track_offset(self):
        ...
        return self._page_up_button or self._page_down_button
    ...
    def _get_minimal_track_offset(self):
        if self._is_linked():
            return ArrangementComponent._minimal_track_offset
        return self.track_offset()

    def _get_minimal_scene_offset(self):
        if self._is_linked():
            return ArrangementComponent._minimal_scene_offset
    ...
    def _is_linked(self):
        return self in ArrangementComponent._linked_session_instances

    def _link(self):
        assert not self._is_linked()
        ArrangementComponent._linked_session_instances.append(self)

    def _unlink(self):
        assert self._is_linked()
        ArrangementComponent._linked_session_instances.remove(self)

    @staticmethod
    def _perform_offset_change(track_increment, scene_increment):
        u""" Performs the given offset changes on all linked instances """
        assert len(ArrangementComponent._linked_session_instances) > 0
        ...
        for instance in ArrangementComponent._linked_session_instances:
        ...
        if not found_negative_offset and instances_covering_session > 0:
            ArrangementComponent._minimal_track_offset = minimal_track_offset
            ArrangementComponent._minimal_scene_offset = minimal_scene_offset
            for instance in ArrangementComponent._linked_session_instances:
